 ...
 *   report is easy to scroll past; a directory with fourteen files in it is not, and a directory
 *   is also something a person can work through and delete from as they go.
 */
import { mkdirSync, rmSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';

import { renderReportMarkdown } from './markdown.js';
import type { ReviewEntry, ZoteroImportReport } from './types.js';
...
export const writeReport = (directory: string, report: ZoteroImportReport): ReportPaths => {
  ...
  // file that has since been found still shows up as missing.
...
const reviewReadme = (report: ZoteroImportReport): string =>
  [
    '# Review queue',
    '',
    `${report.review.length} entr${report.review.length === 1 ? 'y' : 'ies'} from the Zotero import of ` +
      `${report.generatedAt}.`,
    '',
    ...
    'in `../report.md` include every record, whether or not its file was found.',
    '',
...
const reviewEntryMarkdown = (entry: ReviewEntry): string =>
  [
    ...
    '|---|---|',
    `| Kind | ${entry.kind} |`,
    `| Zotero key | ${entry.zoteroKey === null ? '—' : `\`${entry.zoteroKey}\``} |`,
    '',
    '## What happened',
    '',
    entry.reason,
...
const fileNameFor = (entry: ReviewEntry): string => {
  const slug = (entry.zoteroKey ?? entry.subject)
    .normalize('NFKD')
